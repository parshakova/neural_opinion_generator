...
def beam_search(symbols_to_logits_fn,
  ...
  def grow_topk(i, alive_seq, alive_log_probs, rnn_state):
    ...
    flat_logits, rnn_state = symbols_to_logits_fn(flat_ids, rnn_state)
    ...
    rnn_c = rnn_state[0]
    rnn_h = rnn_state[1]
    topk_c = tf.gather_nd(rnn_c, topk_coordinates)
    topk_h = tf.gather_nd(rnn_h, topk_coordinates)
    # select beam of alive states out of 2*beam
    curr_scores = topk_scores + tf.to_float(topk_finished) * -INF
    _, topk_indexes = tf.nn.top_k(curr_scores, k=beam_size)
    batch_pos = compute_batch_indices(batch_size, beam_size)
    top_coordinates = tf.stack([batch_pos, topk_indexes], axis=2)
    rnn_c = tf.gather_nd(topk_c, top_coordinates)
    rnn_h = tf.gather_nd(topk_h, top_coordinates)
    #print("rnn shapes in grow_topk ", rnn_c.get_shape(), rnn_h.get_shape())

    return topk_seq, topk_log_probs, topk_scores, topk_finished, tf.contrib.rnn.LSTMStateTuple(rnn_c, rnn_h)

  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
                 finished_flags, rnn_state):
    """
    ...
    topk_seq, topk_log_probs, topk_scores, topk_finished, rnn_state = grow_topk(
        i, alive_seq, alive_log_probs, rnn_state)
  ...
  def _is_finished(i, unused_alive_seq, alive_log_probs, unused_finished_seq,
                   finished_scores, finished_in_finished, rnn_state):
    """
  ...
  (_, alive_seq, alive_log_probs, finished_seq, finished_scores,
   finished_flags, rnn_state) = tf.while_loop(
       _is_finished,
       inner_loop, [
           tf.constant(0), alive_seq, alive_log_probs, finished_seq,
           finished_scores, finished_flags, rnn_state
       ],
       shape_invariants=[
           tf.TensorShape([]),
           ...
           tf.contrib.rnn.LSTMStateTuple(tf.TensorShape([None, None, rnn_state.c.get_shape().as_list()[2]]), tf.TensorShape([None, None, rnn_state.h.get_shape().as_list()[2]]))
       ],
