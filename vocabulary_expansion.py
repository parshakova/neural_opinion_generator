...
def _load_skip_thoughts_embeddings2(checkpoint_path):
...
def _load_skip_thoughts_embeddings(emb_path):
  word_embedding = np.load(emb_path)
  word_embedding = np.delete(word_embedding,[677444,393736, 918116], 0)
...
def _load_vocabulary(filename):
  ...
  with tf.gfile.GFile(filename, mode="r") as f:
    """
    for i, word in enumerate([u'<pad>',u'<bos>',u'<eos>', u'<oov>', 'N']):
...
def _expand_vocabulary(skip_thoughts_emb, skip_thoughts_vocab, word2vec):
  ...
  shared_words = []
  for w in word2vec:
    if (w in skip_thoughts_vocab):
      shared_words += [w]
    elif (w.title() in skip_thoughts_vocab):
      shared_words += [w.title()]
  ...
  shared_w2v_emb = np.array([word2vec[w.lower()] for w in shared_words])
  print("shapes for LR",shared_w2v_emb.shape, shared_st_emb.shape)
  ...
  print("combined size = ", len(combined_emb))
...
def load_w2v():
    ...
    word_vocab =[u'<pad>',u'<bos>',u'<eos>', u'<oov>', 'N'] + word_vocab[5:]
    
    word_emb_gnews = np.loadtxt("gnews300d_0722.txt")
    word2vec = collections.OrderedDict()
    for idx, char in enumerate(word_vocab):
      word2vec[char]= word_emb_gnews[idx] 
...
def main(unused_argv):
  ...
  #print("before deletion ", skip_thoughts_emb.shape)
  #skip_thoughts_emb = np.delete(skip_thoughts_emb,[677444,393736, 918116], 0)
  skip_thoughts_vocab = _load_vocabulary(FLAGS.skip_thoughts_vocab)
  print(skip_thoughts_emb.shape[0], len(skip_thoughts_vocab))
